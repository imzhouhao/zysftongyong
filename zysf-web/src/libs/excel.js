...
export const export_meta_template_to_excel = (filename, sheetArr) => {
  const wb = XLSX.utils.book_new();
  for (let i = 0, len = sheetArr.length; i <len; i++) {
    let item = sheetArr[i],
      key=item['key'], data=item['data'], title=item['title'],
      sheetName=item['sheetName'], autoWidth=item['autoWidth']
      const arr = json_to_array(key, data);
      arr.unshift(title);
      const ws = XLSX.utils.aoa_to_sheet(arr);
      if(autoWidth){
